...
| #3 TIP DLL 骨架 | In progress | 已新增 `crates/tsf-tip`，可构建 DLL，已导出 COM 入口；注册代码已落地，TSF manager 加载验证仍未完成 |
...
| #4.6 系统加载验证 | TSF manager 创建 TIP 实例 | 切换输入法时出现 `ActivateEx` / `Deactivate` 日志 |
...
- 语言栏切换到该输入法时，TSF manager 调用 `DllGetClassObject` 和 `ActivateEx`。
- 重复注册不会产生重复 profile。
